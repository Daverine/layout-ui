		...
		dropdownInit: function(e) {
			$(this).each(function() {
				...
				if (!dropdown.data("initialized")) {
					...
					if (dropdown.hasClass("select") && !dropdown.children(".content").length) {
					...
					if (dropdown.is(".multiple.select")) {
		...
		showDropdown: function(settings, keyboard) {
			...
			if (dropdown.hasClass("search")) {
				...
				searchBox.on("input."+uniqueId, function(e) {					
					...
					directItems = dropdownMenu.find("> .item, > .items > .item").not(".xhover,.disabled,.selected");
			...
			$(document).on("click."+uniqueId, function(e) {
				...
				if (!$target.closest(dropdown).length && !$target.closest(dropdownMenu).length) {
					if (dropdown.hasClass("select")) { setSelect(); }
					else { dropdown.hideDropdown(settings, true); }
				}
				/* do something when an item is clicked */
				else if ($target.closest(dropdownMenu.find("> .item, > .items > .item").not(".disabled,.xhover")).length) {
				...
				*/
				...
				/* Up and down arrrow key navigation on dropdown menu item. */
					...
					if (!dropdownMenu.find(".dropdown.active").length) {
						...
						/* Enable scroll of oveflow parent to hovered item to make it visible */
						if (lfi.length) {
							var
						...
						if (dropdown.hasClass("select") && !dropdown.hasClass("multiple")) { setSelect(lfi, true); }
					}
			...
			/* A mousemove function to trigger when mouse is moved and inititially on keyboard navigation mode. */
